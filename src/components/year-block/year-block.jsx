...
const YearBlock = ({year, allFlights, yearsRange}) => {
  ...
  const yearPath = ROUTES.detail + `${currentYear}`;
    ...
    <section className="year-block">
      ...
      <div className="year-block__top-wrapper">
        <div className="year-block__year">
          <Link className="year-block__year-link" to={yearPath}>
            {currentYear}
          ...
          <span className="year-block__btn-wrapper">
            <button className="year-block__btn" onClick={onYearUpClick}>
            ...
            <button className="year-block__btn" onClick={onYearDownClick}>
              <svg width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12.001 16a1 1 0 0 1-.64-.23l-6-5a1.002 1.002 0 0 1 1.28-1.54l5.36 4.48 5.36-4.32a.998.998 0 0 1 1.41.15 1.001 1.001 0 0 1-.14 1.46l-6 4.83a1 1 0 0 1-.63.17Z" fill="currentColor" />
              </svg>
            </button>
        ...
        <div className="year-block__info-wrapper">
          <p className="year-block__info">
            ...
            <span>
              {yearStatistic.timeFlight ? <span className="year-block__value year-block__value--plan">{Math.round(yearStatistic.timeFlight / 3600)}</span> : null} {/* {? Время в данных из задания в секундах?} */}
              {yearPlanStatistic.timeFlight ? <span className="year-block__value year-block__value--fact">{`${yearStatistic.timeFlight ? `/` :``}${Math.round(yearPlanStatistic.timeFlight / 3600)}`}</span> : null}
            </span>
          </p>
          <p className="year-block__info">
            <span className="year-block__description">раб.вр.</span>
            <span>
              {yearStatistic.timeWork ? <span className="year-block__value year-block__value--plan">{Math.round(yearStatistic.timeWork / 3600)}</span> : null}
              {yearPlanStatistic.timeWork ? <span className="year-block__value year-block__value--fact">{`${yearStatistic.timeWork ? `/` :``}${Math.round(yearPlanStatistic.timeWork / 3600)}`}</span> : null}
            </span>
          </p>
        </div>
      </div>
        ...
        {monthStatistic.map((it, index) => {
          return (
            <li className="year-block__month" key={`${currentYear}-${MONTHS[index]}`}>
              ...
              <span>
                {it.timeFlight ? <span className="year-block__month-value year-block__month-value--plan">{Math.round(it.timeFlight / 3600)}</span> : null}
                {monthPlanStatistic[index].timeFlight ? <span className="year-block__month-value year-block__month-value--fact">{`${it.timeFlight ? `/` : ``}${Math.round(monthPlanStatistic[index].timeFlight / 3600)}`}</span> : null}
              </span>
              ...
              <span className="year-block__month-description year-block__month-description--work year-block__month-description--mob">раб.время</span>
                <span>
                {it.timeWork ? <span className="year-block__month-value year-block__month-value--plan">{Math.round(it.timeWork / 3600)}</span> : null}
                {monthPlanStatistic[index].timeWork ? <span className="year-block__month-value year-block__month-value--fact">{`${it.timeWork ? `/` : ``}${Math.round(monthPlanStatistic[index].timeWork / 3600)}`}</span> : null}
              </span>
            </li>
          );
        })}
